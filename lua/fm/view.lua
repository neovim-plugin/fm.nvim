...
-- List of bufnrs
---@type table<integer, fm.ViewData>
local session = {}
...
M.rerender_all_fm_buffers = function(opts, callback)
  opts = opts or {}
  local buffers = M.get_all_buffers()
...
local function redraw_trash_virtual_text(bufnr)
  ...
  local ns = vim.api.nvim_create_namespace("FmVtext")
...
M.initialize = function(bufnr)
  ...
  vim.api.nvim_create_autocmd("BufHidden", {
    ...
    callback = function()
      ...
      vim.defer_fn(function()
        ...
        -- Only delete fm buffers if none of them are visible
  ...
  vim.api.nvim_create_autocmd("InsertEnter", {
    desc = "Constrain fm cursor position",
    group = "Fm",
    buffer = bufnr,
    callback = function()
  ...
  then
    ...
    fs_event:start(
      ...
      vim.schedule_wrap(function(err, filename, events)
        ...
        if err or vim.bo[bufnr].modified or vim.b[bufnr].fm_dirty or mutator.is_mutating() then
          return
        end
  ...
  if adapter and adapter.name == "trash" then
    ...
    vim.api.nvim_create_autocmd("TextChanged", {
      desc = "Update fm virtual text of original path",
      buffer = bufnr,
      callback = function()
...
M.format_entry_cols = function(entry, column_defs, col_width, adapter)
  ...
  elseif entry_type == "link" then
    ...
    if link_text then
      table.insert(cols, { link_text, "FmLinkTarget" })
    end
  else
...
M.render_buffer_async = function(bufnr, opts, callback)
  ...
  if vim.b[bufnr].fm_rendering then
  ...
  local handle_error = vim.schedule_wrap(function(message)
    vim.b[bufnr].fm_rendering = false
