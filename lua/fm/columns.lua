...
---@alias fm.ColumnSpec string|table

---@class (exact) fm.ColumnDefinition
---@field render fun(entry: fm.InternalEntry, conf: nil|table): nil|fm.TextChunk
---@field parse fun(line: string, conf: nil|table): nil|string, nil|string
---@field meta_fields? table<string, fun(parent_url: string, entry: fm.InternalEntry, cb: fun(err: nil|string))>
---@field compare? fun(entry: fm.InternalEntry, parsed_value: any): boolean
---@field render_action? fun(action: fm.ChangeAction): string
---@field perform_action? fun(action: fm.ChangeAction, callback: fun(err: nil|string))
---@field get_sort_value? fun(entry: fm.InternalEntry): number|string

---@param name string
---@param column fm.ColumnDefinition
M.register = function(name, column)
  all_columns[name] = column
end

---@param adapter fm.Adapter
---@param defn fm.ColumnSpec
...
---@param adapter_or_scheme string|fm.Adapter
---@return fm.ColumnSpec[]
...
---@return fun(parent_url: string, entry: fm.InternalEntry, cb: fun(err: nil|string))
