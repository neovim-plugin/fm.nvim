---@class (exact) fm.setupOpts
---@field default_file_explorer? boolean Fm will take over directory buffers (e.g. `vim .` or `:e src/`). Set to false if you still want to use netrw.
---@field columns? fm.ColumnSpec[] The columns to display. See :help fm-columns.
---@field buf_options? table<string, any> Buffer-local options to use for fm buffers
---@field win_options? table<string, any> Window-local options to use for fm buffers
---@field delete_to_trash? boolean Send deleted files to the trash instead of permanently deleting them (:help fm-trash).
---@field skip_confirm_for_simple_edits? boolean Skip the confirmation popup for simple operations (:help fm.skip_confirm_for_simple_edits).
...
---@field cleanup_delay_ms? integer Fm will automatically delete hidden buffers after this delay. You can set the delay to false to disable cleanup entirely. Note that the cleanup process only starts when none of the fm buffers are currently displayed.
---@field lsp_file_methods? fm.LspFileMethods Configure LSP file operation integration.
---@field constrain_cursor? false|"name"|"editable" Constrain the cursor to the editable parts of the fm buffer. Set to `false` to disable, or "name" to keep it on the file names.
---@field watch_for_changes? boolean Set to true to watch the filesystem for changes and reload fm.
---@field keymaps? table<string, any>
---@field use_default_keymaps? boolean Set to false to disable all of the above keymaps
---@field view_options? fm.ViewOptions Configure which files are shown and how they are shown.
---@field extra_scp_args? string[] Extra arguments to pass to SCP when moving/copying files over SSH
---@field git? fm.GitOptions EXPERIMENTAL support for performing file operations with git
---@field float? table Configuration for the floating window in fm.open_float
...
---@field sort? fm.SortSpec[] Sort order for the file list

---@class (exact) fm.SortSpec
---@field [1] string
---@field [2] "asc"|"desc"

---@class (exact) fm.GitOptions
