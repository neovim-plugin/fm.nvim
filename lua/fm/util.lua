...
---@alias fm.IconProvider fun(type: string, name: string, conf: table?): (icon: string, hl: string)
...
M.get_adapter = function(bufnr)
  ...
  if not adapter then
    vim.notify_once(
      string.format("[fm] could not find adapter for buffer '%s://'", bufname),
...
M.set_highlights = function(bufnr, highlights)
  local ns = vim.api.nvim_create_namespace("Fm")
...
M.addslash = function(path, os_slash)
  local slash = "/"
  if os_slash and require("fm.fs").is_windows then
    slash = "\\"
  end
...
M.add_title_to_win = function(winid, opts)
  ...
  if title_winid and vim.api.nvim_win_is_valid(title_winid) then
    ...
    elseif opts.align ~= "left" then
      vim.notify(
        string.format("Unknown fm window title alignment: '%s'", opts.align),
        vim.log.levels.ERROR
      )
    end
    ...
    local update_autocmd = vim.api.nvim_create_autocmd("BufWinEnter", {
      desc = "Update fm floating window title when buffer changes",
      pattern = "*",
    ...
    vim.api.nvim_create_autocmd("WinClosed", {
      desc = "Close fm floating window title when floating window closes",
...
---@param action fm.Action
---@return fm.Adapter
---@return nil|fm.CrossAdapterAction
...
M.hide_cursor = function()
  vim.api.nvim_set_hl(0, "FmPreviewCursor", { nocombine = true, blend = 100 })
  local original_guicursor = vim.go.guicursor
  vim.go.guicursor = "a:FmPreviewCursor/FmPreviewCursor"
...
---@param callback fun(err: nil|string, entries: nil|fm.InternalEntry[])
...
---Send files from the current fm directory to quickfix
...
M.send_to_quickfix = function(opts)
  ...
  for i = range.start_lnum, range.end_lnum do
    local entry = fm.get_entry_on_line(0, i)
  ...
  if #qf_entries == 0 then
    vim.notify("[fm] No entries found to send to quickfix", vim.log.levels.WARN)
    return
  end
  vim.api.nvim_exec_autocmds("QuickFixCmdPre", {})
  local qf_title = "fm files"
...
---@param bufnr integer The fm buffer that contains the entry
---@param entry fm.Entry
  ...
  if has_devicons then
    return function(type, name, conf)
      if type == "directory" then
        return conf and conf.directory or "", "FmDirIcon"
      else
