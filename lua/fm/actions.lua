...
M.select = {
  ...
  parameters = {
    ...
    close = {
      type = "boolean",
      desc = "Close the original fm buffer once selection is made",
    },
  },
}
...
M.change_sort = {
  ...
  parameters = {
    sort = {
      type = "fm.SortSpec[]",
      desc = "List of columns plus direction (see |fm.set_sort|) instead of interactive selection",
    },
  },
}
  ...
  callback = function()
    ...
    elseif scheme == "fm-trash://" then
      ...
      if not fs.is_linux then
        local src_bufnr = vim.b.fm_trash_toggle_src
    ...
    vim.b.fm_trash_toggle_src = bufnr
  end,
}

M.send_to_qflist = {
  desc = "Sends files in the current fm directory to the quickfix list, replacing the previous entries.",
  callback = function(opts)
    opts = vim.tbl_deep_extend("keep", opts or {}, {
      target = "qflist",
      action = "r",
...
M.add_to_qflist = {
  desc = "Adds files in the current fm directory to the quickfix list, keeping the previous entries.",
  deprecated = true,
  callback = function()
    util.send_to_quickfix({
      target = "qflist",
...
M.send_to_loclist = {
  desc = "Sends files in the current fm directory to the location list, replacing the previous entries.",
  deprecated = true,
  callback = function()
    util.send_to_quickfix({
      target = "loclist",
...
M.add_to_loclist = {
  desc = "Adds files in the current fm directory to the location list, keeping the previous entries.",
  deprecated = true,
  callback = function()
    util.send_to_quickfix({
      target = "loclist",
