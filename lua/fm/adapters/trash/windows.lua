...
local files = require("fm.adapters.files")
local fs = require("fm.fs")
local powershell_trash = require("fm.adapters.trash.windows.powershell-trash")
...
---@param cb fun(err?: string, entries?: fm.InternalEntry[], fetch_more?: fun())
M.list = function(url, column_defs, cb)
  ...
  powershell_trash.list_raw_entries(function(err, raw_entries)
    ...
    local displayed_entries = vim.tbl_map(
      ---@param entry {IsFolder: boolean, DeletionDate: integer, Name: string, Path: string, OriginalPath: string}
      ---@return {[1]:nil, [2]:string, [3]:string, [4]:{stat: uv_fs_t, trash_info: fm.WindowsTrashInfo, display_name: string}}
      function(entry)
        local parent = win_addslash(assert(vim.fn.fnamemodify(entry.OriginalPath, ":h")))

        --- @type fm.InternalEntry
        local cache_entry
...
---@param name string
---@return nil|fm.ColumnDefinition
...
M.get_entry_path = function(url, entry, cb)
  ...
  local meta = internal_entry[FIELD_META] --[[@as {stat: uv_fs_t, trash_info: fm.WindowsTrashInfo, display_name: string}]]
  local trash_info = meta.trash_info
...
---@param trash_info fm.WindowsTrashInfo
---@param cb fun(err?: string, raw_entries: fm.WindowsRawEntry[]?)
...
---@param path string
---@param type string
---@param cb fun(err?: string, trash_info?: fm.TrashInfo)
...
M.perform_action = function(action, cb)
  if action.type == "delete" then
    local entry = assert(cache.get_entry_by_url(action.url))
    local meta = entry[FIELD_META] --[[@as {stat: uv_fs_t, trash_info: fm.WindowsTrashInfo, display_name: string}]]
    local trash_info = meta.trash_info
