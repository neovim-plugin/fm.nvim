...
local git = require("fm.git")
local permissions = require("fm.adapters.files.permissions")
local trash = require("fm.adapters.files.trash")
...
---@param path string
---@param entry_type nil|fm.EntryType
---@return string
...
local function get_group_ids()
  if not _group_ids then
    ...
    if vim.v.shell_error == 0 then
      ...
      vim.notify(
        "[fm] missing the `id` command. Some directories may not be modifiable even if you have group access.",
        vim.log.levels.WARN
      )
    end
  end
  ...
  elseif action.type == "delete" then
    ...
    if config.delete_to_trash then
      if config.trash_command then
        vim.notify_once(
          "Fm now has native support for trash. Remove the `trash_command` from your config to try it out!",
          vim.log.levels.WARN
        )
        trash.recursive_delete(path, cb)
      else
        require("fm.adapters.trash").delete_to_trash(path, cb)
      end
    else
