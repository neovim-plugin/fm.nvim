...
local permissions = require("fm.adapters.files.permissions")
local shell = require("fm.shell")
local sshfs = require("fm.adapters.ssh.sshfs")
...
---@param fm_url string
---@return fm.sshUrl
M.parse_url = function(fm_url)
  local scheme, url = util.parse_url(fm_url)
  assert(scheme and url, string.format("Malformed input url '%s'", fm_url))
  ...
  if not ret.host or not ret.path then
    error(string.format("Malformed SSH url: %s", fm_url))
  end
...
---@param url1 fm.sshUrl
---@param url2 fm.sshUrl
---@return boolean
...
---@param callback fun(err?: string, entries?: fm.InternalEntry[], fetch_more?: fun())
...
M.read_file = function(bufnr)
  ...
  local tmpdir = fs.join(cache_dir, "fm")
  fs.mkdirp(tmpdir)
