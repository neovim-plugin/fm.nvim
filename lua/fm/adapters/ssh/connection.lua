...
---@class (exact) fm.sshCommand
---@field cmd string|string[]
---@field cb fun(err?: string, output?: string[])
---@field running? boolean

---@class (exact) fm.sshConnection
---@field new fun(url: fm.sshUrl): fm.sshConnection
---@field create_ssh_command fun(url: fm.sshUrl): string[]
...
---@field private commands fm.sshCommand[]
      ...
      vim.defer_fn(function()
        if code == 0 then
          ...
          self:_set_connection_error(
            'Unknown SSH error\nTo see more, run :lua require("fm.adapters.ssh").open_terminal()'
          )
        end
      end, 20)
    end,
