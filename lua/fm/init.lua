...
---@class (exact) fm.Entry
---@field name string
---@field type fm.EntryType
---@field id nil|integer Will be nil if it hasn't been persisted to disk yet
---@field parsed_name nil|string
---@field meta nil|table

---@alias fm.EntryType "file"|"directory"|"socket"|"link"|"fifo"
---@alias fm.HlRange { [1]: string, [2]: integer, [3]: integer } A tuple of highlight group name, col_start, col_end
---@alias fm.HlTuple { [1]: string, [2]: string } A tuple of text, highlight group
---@alias fm.HlRangeTuple { [1]: string, [2]: fm.HlRange[] } A tuple of text, internal highlights
---@alias fm.TextChunk string|fm.HlTuple|fm.HlRangeTuple
---@alias fm.CrossAdapterAction "copy"|"move"

---@class (exact) fm.Adapter
---@field name string The unique name of the adapter (this will be set automatically)
---@field list fun(path: string, column_defs: string[], cb: fun(err?: string, entries?: fm.InternalEntry[], fetch_more?: fun())) Async function to list a directory.
...
---@field get_column fun(name: string): nil|fm.ColumnDefinition If the adapter has any adapter-specific columns, return them when fetched by name.
---@field get_parent? fun(bufname: string): string Get the parent url of the given buffer
---@field normalize_url fun(url: string, callback: fun(url: string)) Before fm opens a url it will be normalized. This allows for link following, path normalizing, and converting an fm file url to the actual path of a file.
---@field get_entry_path? fun(url: string, entry: fm.Entry, callback: fun(path: string)) Similar to normalize_url, but used when selecting an entry
---@field render_action? fun(action: fm.Action): string Render a mutation action for display in the preview window. Only needed if adapter is modifiable.
---@field perform_action? fun(action: fm.Action, cb: fun(err: nil|string)) Perform a mutation action. Only needed if adapter is modifiable.
...
---@field supported_cross_adapter_actions? table<string, fm.CrossAdapterAction> Mapping of adapter name to enum for all other adapters that can be used as a src or dest for move/copy actions.
---@field filter_action? fun(action: fm.Action): boolean When present, filter out actions as they are created
---@field filter_error? fun(action: fm.ParseError): boolean When present, filter out errors from parsing a buffer
...
M.get_entry_on_line = function(bufnr, lnum)
  ...
  if vim.bo[bufnr].filetype ~= "fm" then
    return nil
  end
...
---@param cols fm.ColumnSpec[]
M.set_columns = function(cols)
  require("fm.view").set_columns(cols)
end

---Change the sort order for fm
---@param sort fm.SortSpec[] List of columns plus direction. See :help fm-columns to see which ones are sortable.
---@example
--- require("fm").set_sort({ { "type", "asc" }, { "size", "desc" } })
M.set_sort = function(sort)
  require("fm.view").set_sort(sort)
end
...
M.set_is_hidden_file = function(is_hidden_file)
  require("fm.view").set_is_hidden_file(is_hidden_file)
end
...
---Get the fm url for a given directory
---@private
---@param dir nil|string When nil, use the cwd
---@param use_fm_parent nil|boolean If in an fm buffer, return the parent (default true)
---@return string The parent url
---@return nil|string The basename (if present) of the file/dir we were just in
M.get_url_for_path = function(dir, use_fm_parent)
  if use_fm_parent == nil then
    use_fm_parent = true
  end
  local config = require("fm.config")
  local fs = require("fm.fs")
  ...
  if dir then
    ...
    return M.get_buffer_parent_url(bufname, use_fm_parent)
  end
end

---@private
---@param bufname string
---@param use_fm_parent boolean If in an fm buffer, return the parent
---@return string
---@return nil|string
M.get_buffer_parent_url = function(bufname, use_fm_parent)
  local config = require("fm.config")
  local fs = require("fm.fs")
  local pathutil = require("fm.pathutil")
  local util = require("fm.util")
...
M.open_float = function(dir)
  local config = require("fm.config")
  local layout = require("fm.layout")
  local util = require("fm.util")
  local view = require("fm.view")
  ...
  vim.w[winid].is_fm_win = true
  vim.w[winid].fm_original_win = original_winid
  ...
  table.insert(
    ...
    vim.api.nvim_create_autocmd("BufWinEnter", {
      desc = "Reset local fm window options when buffer changes",
      pattern = "*",
...
---@param fm_bufnr? integer
local function update_preview_window(fm_bufnr)
  fm_bufnr = fm_bufnr or 0
  local util = require("fm.util")
  util.run_after_load(fm_bufnr, function()
    local cursor_entry = M.get_cursor_entry()
    local preview_win_id = util.get_preview_win()
    if
      cursor_entry
      and preview_win_id
      and cursor_entry.id ~= vim.w[preview_win_id].fm_entry_id
    then
      M.open_preview()
    end
  end)
...
M.close = function()
  -- If we're in a floating fm window, close it and try to restore focus to the original window
  if vim.w.is_fm_win then
    local original_winid = vim.w.fm_original_win
  ...
  local ok, bufnr = pcall(vim.api.nvim_win_get_var, 0, "fm_original_buffer")
  if ok and vim.api.nvim_buf_is_valid(bufnr) then
    vim.api.nvim_win_set_buf(0, bufnr)
    if vim.w.fm_original_view then
      vim.fn.winrestview(vim.w.fm_original_view)
    end
    return
  end
...
M.open_preview = function(opts, callback)
  ...
  if util.is_floating_win() then
    if preview_win == nil then
      ...
      vim.api.nvim_win_set_config(0, win_opts_fm)
      local win_opts = {
        relative = "editor",
  ...
  util.get_edit_path(bufnr, entry, function(normalized_url)
    ...
    if entry_is_file and vim.fn.bufloaded(filebufnr) == 0 then
      ...
      vim.b[filebufnr].fm_preview_buffer = true
    end
    ...
    vim.w.fm_entry_id = entry.id
    vim.w.fm_source_win = prev_win
    if is_visual_mode then
...
---@field close? boolean Close the original fm buffer once selection is made

---Select the entry under the cursor
---@param opts nil|fm.SelectOpts
...
M.select = function(opts, callback)
  ...
  if opts.preview then
    vim.notify_once(
      "Deprecated: do not call fm.select with preview=true. Use fm.open_preview instead.\nThis shim will be removed on 2025-01-01"
    )
  ...
  if not adapter then
    return finish("Not an fm buffer")
  end
  ...
  local prev_win = vim.api.nvim_get_current_win()
  local fm_bufnr = vim.api.nvim_get_current_buf()
  ...
  local function open_next_entry(cb)
    ...
    util.get_edit_path(fm_bufnr, entry, function(normalized_url)
      local mods = {
        vertical = opts.vertical,
        horizontal = opts.horizontal,
        split = opts.split,
      ...
      -- So do that for normal files or for fm dirs if config set buflisted=true
  ...
  return {
    {
      name = "FmDir",
      link = "Directory",
      desc = "Directory names in an fm buffer",
    },
    {
      name = "FmDirIcon",
      link = "FmDir",
    ...
    },
    {
      name = "FmSocket",
      link = "Keyword",
      desc = "Socket files in an fm buffer",
    },
    {
      name = "FmLink",
      link = nil,
      desc = "Soft links in an fm buffer",
    },
    {
      name = "FmLinkTarget",
      link = "Comment",
    ...
    },
    {
      name = "FmFile",
      link = nil,
      desc = "Normal files in an fm buffer",
    },
    {
      name = "FmCreate",
      link = "DiagnosticInfo",
      desc = "Create action in the fm preview window",
    },
    {
      name = "FmDelete",
      link = "DiagnosticError",
      desc = "Delete action in the fm preview window",
    },
    {
      name = "FmMove",
      link = "DiagnosticWarn",
      desc = "Move action in the fm preview window",
    },
    {
      name = "FmCopy",
      link = "DiagnosticHint",
      desc = "Copy action in the fm preview window",
    },
    {
      name = "FmChange",
      link = "Special",
      desc = "Change action in the fm preview window",
    },
    {
      name = "FmRestore",
      link = "FmCreate",
      desc = "Restore (from the trash) action in the fm preview window",
    },
    {
      name = "FmPurge",
      link = "FmDelete",
      desc = "Purge (Permanently delete a file from trash) action in the fm preview window",
    },
    {
      name = "FmTrash",
      link = "FmDelete",
      desc = "Trash (delete a file to trash) action in the fm preview window",
    },
    {
      name = "FmTrashSourcePath",
      link = "Comment",
...
local function restore_alt_buf()
  if vim.bo.filetype == "fm" then
    require("fm.view").set_win_options()
    vim.api.nvim_win_set_var(0, "fm_did_enter", true)
  elseif vim.w.fm_did_enter then
    vim.api.nvim_win_del_var(0, "fm_did_enter")
    -- We are entering a non-fm buffer *after* having been in an fm buffer
    local has_orig, orig_buffer = pcall(vim.api.nvim_win_get_var, 0, "fm_original_buffer")
    if has_orig and vim.api.nvim_buf_is_valid(orig_buffer) then
      if vim.api.nvim_get_current_buf() ~= orig_buffer then
        -- If we are editing a new file after navigating around fm, set the alternate buffer
        -- to be the last buffer we were in before opening fm
        vim.fn.setreg("#", orig_buffer)
      else
        -- If we are editing the same buffer that we started fm from, set the alternate to be
        -- what it was before we opened fm
        local has_orig_alt, alt_buffer =
          pcall(vim.api.nvim_win_get_var, 0, "fm_original_alternate")
...
---@param bufnr integer
local function load_fm_buffer(bufnr)
  local config = require("fm.config")
...
local function close_preview_window_if_not_in_fm()
  local util = require("fm.util")
  local preview_win_id = util.get_preview_win()
  if not preview_win_id or not vim.w[preview_win_id].fm_entry_id then
    return
  end

  local fm_source_win = vim.w[preview_win_id].fm_source_win
  if fm_source_win and vim.api.nvim_win_is_valid(fm_source_win) then
    local src_buf = vim.api.nvim_win_get_buf(fm_source_win)
    if util.is_fm_bufnr(src_buf) then
      return
    end
  end
...
local _on_key_ns = 0
---Initialize fm
---@param opts fm.setupOpts|nil
M.setup = function(opts)
  local Ringbuf = require("fm.ringbuf")
  local config = require("fm.config")
  ...
  vim.api.nvim_create_user_command("Fm", function(args)
    ...
    if trash then
      ...
      path = "fm-trash://" .. new_path
    end
    M[method](path)
  end, { desc = "Open fm file browser on a directory", nargs = "*", complete = "dir" })
  local aug = vim.api.nvim_create_augroup("Fm", {})
  ...
  for scheme in pairs(config.adapters) do
    ...
    filetype_patterns[scheme .. ".*"] = { "fm", { priority = 10 } }
  end
  ...
  -- by other patterns. See https://github.com/stevearc/fm.nvim/issues/47
  vim.filetype.add({
  ...
  vim.api.nvim_create_autocmd("ColorScheme", {
    desc = "Set default fm highlights",
    group = aug,
    pattern = "*",
    callback = set_colors,
  })
  ...
  vim.api.nvim_create_autocmd("BufLeave", {
    ...
    callback = function()
      local util = require("fm.util")
      if not util.is_fm_bufnr(0) then
        vim.w.fm_original_buffer = vim.api.nvim_get_current_buf()
        vim.w.fm_original_view = vim.fn.winsaveview()
        ---@diagnostic disable-next-line: param-type-mismatch
        vim.w.fm_original_alternate = vim.fn.bufnr("#")
      end
    end,
  })
  vim.api.nvim_create_autocmd("BufEnter", {
    desc = "Set/unset fm window options and restore alternate buffer",
    group = aug,
    pattern = "*",
    callback = function()
      local util = require("fm.util")
      ...
      if scheme and config.adapters[scheme] then
        ...
        -- While we are in an fm buffer, set the alternate file to the buffer we were in prior to
        -- opening fm
        local has_orig, orig_buffer = pcall(vim.api.nvim_win_get_var, 0, "fm_original_buffer")
        if has_orig and vim.api.nvim_buf_is_valid(orig_buffer) then
          vim.fn.setreg("#", orig_buffer)
        ...
        vim.w.fm_did_enter = true
      elseif vim.fn.isdirectory(bufname) == 0 then
        -- Only run this logic if we are *not* in an fm buffer (and it's not a directory, which
        -- will be replaced by an fm:// url)
        -- Fm buffers have to run it in BufReadCmd after confirming they are a directory or a file
        restore_alt_buf()
      end

      close_preview_window_if_not_in_fm()
    end,
  })
  ...
  vim.api.nvim_create_autocmd({ "BufWinEnter", "WinNew", "WinEnter" }, {
    ...
    callback = function()
      ...
      if vim.b.fm_preview_buffer and not vim.wo.previewwindow then
        vim.bo.bufhidden = vim.api.nvim_get_option_value("bufhidden", { scope = "global" })
        vim.b.fm_preview_buffer = nil
      end
    end,
  })
  if not config.silence_scp_warning then
    vim.api.nvim_create_autocmd("BufNew", {
      ...
      callback = function()
        vim.notify(
          "If you are trying to browse using Fm, use fm-ssh:// instead of scp://\nSet `silence_scp_warning = true` in fm.setup() to disable this message.\nSee https://github.com/stevearc/fm.nvim/issues/27 for more information.",
          vim.log.levels.WARN
        )
      end,
    })
  ...
  vim.api.nvim_create_autocmd("WinNew", {
    desc = "Restore window options when splitting an fm window",
    group = aug,
    pattern = "*",
    nested = true,
    callback = function(params)
      local util = require("fm.util")
      if not util.is_fm_bufnr(params.buf) or vim.w.fm_did_enter then
        return
      end
      -- This new window is a split off of an fm window. We need to transfer the window
      ...
      for _, winid in ipairs(winids) do
        if vim.api.nvim_win_is_valid(winid) then
          if vim.w[winid].fm_did_enter then
            parent_win = winid
            break
          end
        end
      ...
      if not parent_win then
        vim.notify(
          "Fm split could not find parent window. Please try to replicate whatever you just did and report a bug on github",
          vim.log.levels.WARN
        )
        return
      end
      ...
      vim.w.fm_did_enter = true
      vim.w.fm_original_buffer = vim.w[parent_win].fm_original_buffer
      vim.w.fm_original_view = vim.w[parent_win].fm_original_view
      vim.w.fm_original_alternate = vim.w[parent_win].fm_original_alternate
    end,
  })
  vim.api.nvim_create_autocmd("BufAdd", {
    desc = "Detect directory buffer and open fm file browser",
    group = aug,
    pattern = "*",
    nested = true,
    callback = function(params)
  ...
  -- mksession doesn't save fm buffers in a useful way. We have to manually load them after a
  -- session finishes loading. See https://github.com/stevearc/fm.nvim/issues/29
  vim.api.nvim_create_autocmd("SessionLoadPost", {
    desc = "Load fm buffers after a session is loaded",
    group = aug,
    pattern = "*",
    callback = function(params)
