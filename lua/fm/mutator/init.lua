...
local Trie = require("fm.mutator.trie")
...
local lsp_helpers = require("fm.lsp.helpers")
local fm = require("fm")
local parser = require("fm.mutator.parser")
local preview = require("fm.mutator.preview")
...
---@alias fm.Action fm.CreateAction|fm.DeleteAction|fm.MoveAction|fm.CopyAction|fm.ChangeAction

---@class (exact) fm.CreateAction
---@field type "create"
---@field url string
---@field entry_type fm.EntryType
---@field link nil|string

---@class (exact) fm.DeleteAction
---@field type "delete"
---@field url string
---@field entry_type fm.EntryType

---@class (exact) fm.MoveAction
---@field type "move"
---@field entry_type fm.EntryType
---@field src_url string
---@field dest_url string

---@class (exact) fm.CopyAction
---@field type "copy"
---@field entry_type fm.EntryType
---@field src_url string
---@field dest_url string

---@class (exact) fm.ChangeAction
---@field type "change"
---@field entry_type fm.EntryType
---@field url string
---@field column string
...
---@param all_diffs table<integer, fm.Diff[]>
---@return fm.Action[]
M.create_actions_from_diffs = function(all_diffs)
  ---@type fm.Action[]
  local actions = {}

  ---@type table<integer, fm.Diff[]>
...
M.enforce_action_order = function(actions)
  ...
  ---@return nil|fm.Action The leaf action
  ---@return nil|fm.Action When no leaves found, this is the last action in the loop
    ...
    if selected then
      ...
      if loop_action and loop_action.type == "move" then
        ...
        local intermediate_url =
          string.format("%s__fm_tmp_%05d", loop_action.src_url, math.random(999999))
        local move_1 = {
          type = "move",
...
M.process_actions = function(actions, cb)
  vim.api.nvim_exec_autocmds(
    "User",
    { pattern = "FmActionsPre", modeline = false, data = { actions = actions } }
  )
  ...
  local function finish(err)
    if not finished then
      ...
      vim.api.nvim_exec_autocmds(
        "User",
        { pattern = "FmActionsPost", modeline = false, data = { err = err, actions = actions } }
      )
      cb(err)
    end
  end
...
M.try_write_changes = function(confirm, cb)
  ...
  local all_diffs = {}
  ---@type table<integer, fm.ParseError[]>
  local all_errors = {}
  ...
  preview.show(actions, confirm, function(proceed)
    ...
    M.process_actions(
      ...
      vim.schedule_wrap(function(err)
        view.unlock_buffers()
        if err then
          err = string.format("[fm] Error applying actions: %s", err)
          view.rerender_all_fm_buffers(nil, function()
            cb(err)
          end)
        else
          local current_entry = fm.get_cursor_entry()
          ...
          view.rerender_all_fm_buffers(nil, function(render_err)
            vim.api.nvim_exec_autocmds(
              "User",
              { pattern = "FmMutationComplete", modeline = false }
            )
            cb(render_err)
          end)
        end
